...
| **ConnectTimeout**        | Optional          | `string` | Parameter to specify the TimeSpan to wait when the adapter is trying to connect to the data source. The value ranges from `1` second to `30` seconds, represented as TimeSpan strings 00:00:01 and 00:00:30, respectively. The default value is `5` seconds, represented as 00:00:05. **\***|
| **ReconnectInterval**     | Optional          | `string` | Parameter to specify the TimeSpan to wait before retrying to connect to the data source when the data source is offline. The value ranges from `100` ms to `30` sec, represented as TimeSpan strings 00:00:00.1 and 00:00:30, respectively. The default value is `1` second, represented as 00:00:01. **\*** |
|**RequestTimeout**         | Optional          | `string` | Parameter to specify the TimeSpan that the adapter waits for a pending request before marking it as timeout and dropping the request. The default value is `10` seconds, represented as the TimeSpan string 00:00:10. The value must be positive. There is no value range. **\***|
|**DelayBetweenRequests**   | Optional          | `string` | Parameter to specify the minimum TimeSpan between two successive requests sent to the data source. The value ranges from `0` seconds to `1` second, represented as TimeSpan strings 00:00:00 and 00:00:01, respectively. The default value is `0` seconds, represented as 00:00:00. **\***|
...
**\* Note:** You can also specify timespans as numbers in seconds. For example, `"RequestTimeout": 25` specifies 25 seconds, or `"RequestTimeout": 125` specifies 2 minutes and 5 seconds.
